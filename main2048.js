/**
 * Created by mizeshuang on 2016/5/14.
 */
var board = new Array();
var score = 0;
var hasConflicted = new Array();// 用来判断两个格子之间是否已经发生过碰撞，从而避免一下子加好几个格子，让加过的格子不进行加操作
...
function init() {
    ...
    /*
    * 初始化数组，board是存储数据，hasConflicted判断是否发生过碰撞
    * */
    for (var i = 0; i < 4; i++) {
        board[i] = new Array();
...
function updateBoardView() {
    $(".number-cell").remove();//选中所有类属性为.number-cell的标签进行删除操作
    for (var i = 0; i < 4; i++)
        for (var j = 0; j < 4; j++) {
            $("#grid-container").append('<div class="number-cell" id="number-cell-' + i + '-' + j + '"></div>');
            var theNumberCell = $('#number-cell-' + i + '-' + j);//获得
...
function generateOneNumber() {
    ...
    if (nospace(board))//判断board的数组是否为全满的
        return false;
...
$(document).keydown(function (event) {
    event.preventDefault();//阻挡上下键对浏览器的影响，阻止滚动条的移动
...
function moveLeft() {
    ...
    for (var i = 0; i < 4; i++)
        for (var j = 1; j < 4; j++) {
            if (board[i][j] != 0) {
                for (var k = 0; k < j; k++) {
                    if (board[i][k] == 0 && noBlockHorizontal(i, k, j, board)) {//处理前面没有数字
                        //move
